...
data <- read_csv("/Users/carlpearson/Documents/r_github/openshift_top_tasks/do_not_upload/six_weeks.csv",col_names = T)
...
df.wide <-  df %>% 
  ...
  na.omit(response) %>% #remove na, unpicked choices
...
colnames(df.cli.rank) <- df.cli.rank[1,]

df.cli.rank <- data %>% 
  filter(Status != "Survey Preview",#remove previews
         RH =="No, I'm not a Red Hat employee.") %>%  #remove internals
  select(ResponseId,contains("cli_topt"),contains("cli_rank"))
